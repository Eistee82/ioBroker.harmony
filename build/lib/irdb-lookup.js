...
exports.lookupIRCode = lookupIRCode;
exports.narrowCandidates = narrowCandidates;
 ...
 */
function parseCSV(csv, manufacturer, deviceType) {
 ...
 */
async function lookupIRCode(protocol, device, subdevice, func, log) {
    const matches = [];
    ...
    try {
        ...
        if (log)
            log(`Searching irdb for protocol=${protocol} device=${device} subdevice=${subdevice} function=${func}`);
        // Search in parallel across manufacturers and device types
        const fetchPromises = [];
        for (const mfr of manufacturers) {
            for (const dt of deviceTypes) {
                const url = `${IRDB_CDN}/codes/${mfr}/${dt}/${searchFile}`;
                fetchPromises.push(fetchUrl(url, 5000)
                    .then((csv) => {
 ...
 */
function narrowCandidates(allMatches) {
...
//# sourceMappingURL=irdb-lookup.js.map
...
function match(actual: number, expected: number, tolerance = 0.25): boolean {
    return Math.abs(actual - expected) <= expected * tolerance;
}

/** Decode a single bit from NEC-style encoding */
function necBit(mark: number, space: number): number | null {
    if (!match(mark, 562, 0.3)) return null;
    if (match(space, 562, 0.3)) return 0;
    if (match(space, 1687, 0.3)) return 1;
    return null;
}

/** Decode bits from timing array starting at offset */
function decodeBits(timings: number[], startIdx: number, count: number, bitFn: (m: number, s: number) => number | null): { value: number; nextIdx: number } | null {
    let value = 0;
    let idx = startIdx;
    for (let i = 0; i < count; i++) {
        if (idx + 1 >= timings.length) return null;
...
function decodeNEC(timings: number[]): DecodedIR | null {
    if (timings.length < 67) return null; // 2 header + 64 data + 1 trail minimum

    // Check header: 9000us mark, 4500us space
    if (!match(timings[0], 9000, 0.25) || !match(timings[1], 4500, 0.25)) return null;
    ...
    if ((cmd ^ cmdInv) !== 0xFF) return null;
...
function decodeSamsung(timings: number[]): DecodedIR | null {
    if (timings.length < 67) return null;

    // Samsung header: ~4500us mark, ~4500us space
    if (!match(timings[0], 4500, 0.25) || !match(timings[1], 4500, 0.25)) return null;
...
function decodeSony(timings: number[]): DecodedIR | null {
    if (timings.length < 25) return null;

    // Header: 2400us mark, 600us space
    if (!match(timings[0], 2400, 0.25) || !match(timings[1], 600, 0.3)) return null;
    ...
    let cmd = 0;
    for (let i = 0; i < 7; i++) cmd |= bits[i] << i;

    let device = 0;
    const addrBits = bits.length - 7;
    for (let i = 0; i < addrBits; i++) device |= bits[7 + i] << i;
...
function decodeRC5(timings: number[]): DecodedIR | null {
...
function decodeLG(timings: number[]): DecodedIR | null {
    if (timings.length < 65) return null;

    // LG header: 8500us mark, 4250us space
    if (!match(timings[0], 8500, 0.25) || !match(timings[1], 4250, 0.25)) return null;
 ...
 */
export function parseHarmonyIRData(raw: string): number[] {
 ...
 */
export function decodeIR(timings: number[]): DecodedIR | null {
